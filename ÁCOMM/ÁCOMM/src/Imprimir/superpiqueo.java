...
import Formulario.Piqueo;
import libreria.superBebida;
import libreria.superConsulta;

public class superpiqueo extends JFrame  implements MouseListener, MouseMotionListener, ActionListener, KeyListener,Printable{
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					superpiqueo frame = new superpiqueo();
	...
	public  superpiqueo() {
		...
		lblNewLabel.setIcon(new ImageIcon(superpiqueo.class.getResource("/img/piqueo.png")));
		lblNewLabel.setBounds(911, 5, 48, 41);
		...
		lblN.setIcon(new ImageIcon(superpiqueo.class.getResource("/img/cuadroverde.png")));
		...
		ImageIcon fondo11=new ImageIcon(getClass().getResource("/img/pique20.png"));
		...
		txtc5.setBackground(new Color(0, 128, 0));
		...
		txtc3.setBackground(new Color(255, 0, 0));
		...
		txtc2.setBackground(new Color(65, 105, 225));
		...
		txtc.setBackground(new Color(255, 165, 0));
		...
		btndatos = new JButton("Datos de Piqueos");
		...
		btndatalle = new JButton("Detalle de Piqueos");
		...
		panelPastel.setVisible(false);
			...
			cmbMaterno.setBounds(516, 78, 136, 28);
			...
			label_Mater.setBounds(395, 36, 382, 98);
			...
			lblSearchOne.setBounds(98, 11, 98, 14);
			...
			lblSearchTwo.setBounds(469, 11, 85, 14);
			...
			lblSupervisor = new JLabel("Calientes");
			...
			lblAdministradores = new JLabel("Frios");
		...
		lblDetalles = new JLabel("Detalles de Piqueos");
		...
		txtAdministracion.setBackground(new Color(65, 105, 225));
		...
		txtMozo.setBackground(new Color(0, 128, 0));
		txtMozo.setColumns(10);
		txtMozo.setBounds(34, 140, 188, 9);
		paneltabla.add(txtMozo);
		
		lblCantidadDeAdministradores = new JLabel("Cantidad de Frios");
		...
		lblCantidadDeSupervisores = new JLabel("Cantidad de Calientes");
		...
		lblCantidadDeMozos = new JLabel("Cantidad de Snacks");
		...
		lblCantidadDeUsuarios = new JLabel("Piqueos");
		...
		ImageIcon fondo14=new ImageIcon(getClass().getResource("/img/piqueo.png"));
	...
	d.addRow(new Object[]{r.getInt(1),r.getString(2),r.getString(3),r.getDouble(4),r.getString(5)});
		tables.setModel(d);
		
		h2.put(r.getString(2),new clasePiqueo(r.getInt(1),r.getString(2),r.getString(3),r.getDouble(4),r.getString(5)));
		h3.put(r.getString(3),new clasePiqueo(r.getInt(1),r.getString(2),r.getString(3),r.getDouble(4),r.getString(5)));
	...
	d2.addRow(new Object[]{r.getInt(1),r.getString(2),r.getString(3),r.getDouble(4),r.getString(5)});
		table2.setModel(d2);
	...
	private HashMap<String,clasePiqueo> h2=new HashMap<>();
	private HashMap<String,clasePiqueo> h3=new HashMap<>();
	...
	c=(Connection)DriverManager.getConnection("jdbc:mysql://localhost:3306/acomm","root","mysql");
		...
		try {
			...
			ResultSet r=s.executeQuery("select count(u.tipo_piqueo) from piqueo u where u.tipo_piqueo='Frios'");
...
private void contarSupervisor(){
	...
	c=(Connection)DriverManager.getConnection("jdbc:mysql://localhost:3306/acomm","root","mysql");
		...
		try {
			...
			Statement s=(Statement) c.createStatement();
			ResultSet r=s.executeQuery("select count(u.tipo_piqueo) from piqueo u where u.tipo_piqueo='Calientes'");
	...
	c=(Connection)DriverManager.getConnection("jdbc:mysql://localhost:3306/acomm","root","mysql");
		...
		try {
			...
			Statement s=(Statement) c.createStatement();
			ResultSet r=s.executeQuery("select count(u.tipo_piqueo) from piqueo u where u.tipo_piqueo='Snacks'");
	...
	protected void actionPerformedBtnactulizar(ActionEvent arg0) {
		...
		b.setVisible(true);
		b.setLocationRelativeTo(null);
		Piqueo.lbccodigo.setText(Integer.parseInt(txtC.getText())+"");
	...
	c=(Connection)DriverManager.getConnection("jdbc:mysql://localhost:3306/acomm","root","mysql");
			...
			Statement s=(Statement) c.createStatement();
		ResultSet r=s.executeQuery("select b.nombre,b.precio from piqueo b");
	...
	protected void actionPerformedBtnword(ActionEvent arg0) {
		...
		String titulogrande ="Registro de Piqueos";
		...
		r2.setBold(true);
		r2.setText("ID            Nombre            Tipo           Precio             Estado          " );
...
try {
	...
	private void excel(){
		...
		try {
			FileOutputStream f=new FileOutputStream(new File("piqueos.xls"));
			libros.write(f);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void actionPerformedBtnexcel(ActionEvent arg0) {
		...
		try {
			...
			Desktop.getDesktop().open(new File("piqueos.xls"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void actionPerformedBtncorreo(ActionEvent arg0) {
		...
		try {
	        ...
	        String asunto="Registro de Piqueos";
	        String mensaje="Hola<br>Resiviste la Informacion de los Piqueos</br>";
	        ...
	        adjunto.setDataHandler(new DataHandler(new FileDataSource("piqueos.xls")));
	        adjunto.setFileName("Piqueos.xls");
	...
	protected void actionPerformedBtnpdf(ActionEvent arg0) {
		...
		MessageFormat footer=new MessageFormat("Detalle Piqueos{0,number,integer}");
		try {
...
private void buscarnivel2(){
	...
	c=(Connection)DriverManager.getConnection("jdbc:mysql://localhost:3306/acomm","root","mysql");
		...
		try {
			...
			Statement s=(Statement) c.createStatement();
			ResultSet r=s.executeQuery("select u.nombre,u.tipo_piqueo  from piqueo u");
			...
			while(r.next()){
				...
				cmbMaterno.addItem(r.getString(2));
			...
			JOptionPane.showMessageDialog(null, "no hay piqueos");
		}
	...
	protected void mouseClickedCmbN(MouseEvent arg0) {
			d.setRowCount(0);
		if(h2.containsKey(cmbN.getSelectedItem().toString())){
			
			clasePiqueo cl=h2.get(cmbN.getSelectedItem().toString());


			d.addRow(new Object[]{cl.getIDpiqueo(),cl.getNombres(),cl.getTipoPiqueo(),cl.getPrecio(),
					cl.getEstado()});
			tables.setModel(d);
	...
	protected void mouseClickedCmbMaterno(MouseEvent arg0) {
		...
		if(h3.containsKey(cmbMaterno.getSelectedItem().toString())){
				...
				clasePiqueo cl=h3.get(cmbMaterno.getSelectedItem().toString());
