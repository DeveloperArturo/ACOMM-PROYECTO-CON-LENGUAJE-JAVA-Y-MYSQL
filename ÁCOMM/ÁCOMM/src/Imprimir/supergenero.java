...
import Formulario.Genero;
import libreria.superConsulta;
import libreria.superartista;
import libreria.supercancion;

public class supergenero extends JFrame implements MouseListener, MouseMotionListener, ActionListener, KeyListener  {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					supergenero frame = new supergenero();
	...
	public  supergenero() {
		...
		lblNewLabel.setIcon(new ImageIcon(supergenero.class.getResource("/img/genero.png")));
		...
		lblN.setIcon(new ImageIcon(supergenero.class.getResource("/img/cuadrozul.png")));
		...
		txtc6.setBackground(new Color(148, 0, 211));
		...
		txtc.setBackground(new Color(255, 140, 0));
		...
		label.setIcon(new ImageIcon(supergenero.class.getResource("/img/osito2.png")));
		...
		btndatos = new JButton("Datos de Genero\r\n");
		...
		panelgraficos.setBorder(new LineBorder(new Color(0, 0, 0), 0));
		panelgraficos.setBounds(503, 170, 452, 431);
		paneltabla.add(panelgraficos);
	...
	h2.put(r.getString(4),new claseGenero(r.getInt(1),r.getString(2),r.getString(3),r.getString(4),r.getString(5)));
	
		
		lbgenero.setText(String.valueOf(d.getRowCount()));
	...
	private HashMap<String,claseGenero> h2=new HashMap<>();
	...
	c=(Connection)DriverManager.getConnection("jdbc:mysql://localhost:3306/acomm","root","mysql");
		...
		try {
			...
			ResultSet r=s.executeQuery("select count(u.cod_artista) from artista u ");
...
private void contarSupervisor(){
	...
	protected void actionPerformedBtnactulizar(ActionEvent arg0) {
		...
		a.setVisible(true);
		a.setLocationRelativeTo(null);
		Genero.lbccodigo.setText(Integer.parseInt(txtC.getText())+"");
...
ResultSetImpl r=(ResultSetImpl) s.executeQuery("select count(g.epoca) from genero g where g.epoca='1930'");
...
ResultSetImpl r=(ResultSetImpl) s.executeQuery("select count(g.epoca) from genero g where g.epoca='1970'");
...
ResultSetImpl r=(ResultSetImpl) s.executeQuery("select count(g.epoca) from genero g where g.epoca='1980'");
...
ResultSetImpl r=(ResultSetImpl) s.executeQuery("select count(g.epoca) from genero g where g.epoca='1990'");
...
ResultSetImpl r=(ResultSetImpl) s.executeQuery("select count(g.epoca) from genero g where g.epoca='2000'");
...
ResultSetImpl r=(ResultSetImpl) s.executeQuery("select count(g.epoca) from genero g where g.epoca='2015'");
...
private void graficoPastel(){
   ...
   defaultpiedataset.setValue("1930("+uno+")",uno); 
   defaultpiedataset.setValue("1970("+dos+")",dos); 
    defaultpiedataset.setValue("1980("+tres+")",tres); 
    defaultpiedataset.setValue("1990("+cuatro+")",cuatro); 
    defaultpiedataset.setValue("2000("+cinco+")",cinco); 
    defaultpiedataset.setValue("2010("+seis+")",seis); 
    defaultpiedataset.setValue("2015("+siete+")",siete); 
	...
	protected void actionPerformedBtnword(ActionEvent arg0) {
		...
		String titulogrande ="Registro de Genero";
		...
		r2.setBold(true);
		r2.setText("ID            Descripcion            Procedencia            Epoca           Estado" );
...
try {
	...
	private void excel(){
		...
		try {
			FileOutputStream f=new FileOutputStream(new File("genero.xls"));
			libros.write(f);
	...
	protected void actionPerformedBtnexcel(ActionEvent arg0) {
		...
		try {
			...
			Desktop.getDesktop().open(new File("genero.xls"));
		} catch (Exception e) {
			e.printStackTrace();
		}
...
try {
	        ...
	        String mensaje="Hola<br>Resiviste la Informacion de los Generos</br>";
	        ...
	        adjunto.setDataHandler(new DataHandler(new FileDataSource("genero.xls")));
	        adjunto.setFileName("Genero.xls");
