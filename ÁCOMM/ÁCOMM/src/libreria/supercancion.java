...
public class supercancion extends JFrame  implements MouseListener, MouseMotionListener, ActionListener, KeyListener {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					supercancion frame = new supercancion();
	...
	public   supercancion() {
		...
		lblNewLabel.setIcon(new ImageIcon(supercancion.class.getResource("/img/canc.png")));
		...
		lblConsultar.setForeground(Color.GRAY);
		...
		lblN.setIcon(new ImageIcon(supercancion.class.getResource("/img/titulo.png")));
		...
		ImageIcon fondo11=new ImageIcon(getClass().getResource("/img/cancion3.png"));
		...
		btndatos = new JButton("Datos de Canciones");
		...
		panelexportar.setBounds(10, 11, 905, 252);
		...
		lbgenero.setBounds(376, 77, 68, 23);
		...
		d.addColumn("ID");
		d.addColumn("Nombres");
		d.addColumn("Duracion");
		d.addColumn("Artista");
		d.addColumn("Album");
		d.addColumn("Catalogo");
		d.addColumn("Estado");
		...
		btnactulizar.setBounds(515, 119, 210, 69);
		...
		lblActualizar.setBounds(167, 11, 143, 23);
		...
		calendar.setWeekdayForeground(new Color(255, 140, 0));
		...
		calendar.setBounds(503, 274, 412, 327);
		...
		label_2.setBounds(119, 159, 208, 93);
		...
		lblArtistas.setBounds(232, 216, 78, 23);
		...
		btnañadir.setIcon(new ImageIcon(supercancion.class.getResource("/img/a\u00F1adirazul.png")));
		...
		btnañadir.setBounds(735, 119, 180, 69);
		paneltabla.add(btnañadir);
		...
		lblDesarrollo.setBounds(682, 11, 91, 23);
	...
	d.addRow(new Object[]{r.getInt(1),r.getString(2),r.getString(3),r.getString(4),r.getString(5),r.getString(6),r.getString(7)});
		tables.setModel(d);
		
	h2.put(r.getString(6),new clasecancion(r.getInt(1),r.getString(2),r.getString(3),r.getString(4),r.getString(5),r.getString(6),r.getString(7)));
	
		
		lbcancion.setText(String.valueOf(d.getRowCount()));
	...
	private HashMap<String,clasecancion> h2=new HashMap<>();
...
private void contarSupervisor(){
	...
	c=(Connection)DriverManager.getConnection("jdbc:mysql://localhost:3306/acomm","root","mysql");
		...
		try {
			...
			Statement s=(Statement) c.createStatement();
			ResultSet r=s.executeQuery("select count(a.cod_artista) from artista a ");
			
			
			if(r.next()){
			
				lbartista.setText(r.getInt(1)+"");
	...
	protected void actionPerformedBtnactulizar(ActionEvent arg0) {
		...
		c.setVisible(true);
		c.setLocationRelativeTo(null);
		Cancion.lbccodigo.setText(Integer.parseInt(txtC.getText())+"");
...
private void buscarnivel2(){
	...
	protected void actionPerformedBtnañadir(ActionEvent arg0) {
		...
		AñadirCancion a=new AñadirCancion();
		a.setVisible(true);
...
String fecha = new SimpleDateFormat("dd/MM/yyyy").format(calendar.getDate());
		...
		if(h2.containsKey(fecha)){
			...
			clasecancion cc=h2.get(fecha);
			
			d.setRowCount(0);
			d.addRow(new Object[]{cc.getIDCancion(),cc.getNombre(),cc.getDuracion(),cc.getIDArtista(),
					cc.getAlbum(),cc.getRegistroCatalogo(),cc.getEstado()});
			tables.setModel(d);
	...
	protected void actionPerformedBtnpdf(ActionEvent arg0) {
		...
		MessageFormat footer=new MessageFormat("Detalle Canciones{0,number,integer}");
		try {
	...
	protected void actionPerformedBtnword(ActionEvent arg0) {
		...
		String titulogrande ="Registro de Cancion";
		...
		r2.setBold(true);
		r2.setText("ID            Nombre            Duracion            Artista           Album               Catalogo            Estado " );
	...
	tableOneRowOne.addNewTableCell().setText(tables.getValueAt(i,5).toString());
	tableOneRowOne.addNewTableCell().setText(tables.getValueAt(i,6).toString());
...
try {
	...
	private void excel(){
		...
		celda.setCellValue("Artista");
		
		celda=fila.createCell(4);
		celda.setCellValue("Album");
		
		celda=fila.createCell(5);
		celda.setCellValue("Catalogo");
		...
		for (int i = 0; i < tables.getRowCount(); i++) {
			...
			for (int j = 0; j < tables.getRowCount(); j++) {
				 ...
				 celda=fila.createCell(5);
				celda.setCellValue(tables.getValueAt(f,5).toString());
				
				}
			for (int j = 0; j < tables.getRowCount(); j++) {

				 celda=fila.createCell(6);
				celda.setCellValue(tables.getValueAt(f,6).toString());
		...
		try {
			FileOutputStream f=new FileOutputStream(new File("cancion.xls"));
			libros.write(f);
	...
	protected void actionPerformedBtnexcel(ActionEvent arg0) {
		...
		try {
			...
			Desktop.getDesktop().open(new File("cancion.xls"));
		} catch (Exception e) {
			e.printStackTrace();
		}
...
try {
	        ...
	        String mensaje="Hola<br>Resiviste la Informacion de las Canciones</br>";
	        ...
	        adjunto.setDataHandler(new DataHandler(new FileDataSource("cancion.xls")));
	        adjunto.setFileName("cancion.xls");
